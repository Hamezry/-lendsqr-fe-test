...
export const checkErrors = (columns: Column[]) => {
  ...
  const stickyColumnsWithoutGroup = columns
    .filter((column: Column) => column.sticky && !column.parent)
    .map(({ Header }) => `'${Header}'`);
...
function getStickyValue(column: Column): null | 'left' | 'right' {
...
export function columnIsFirstRightSticky(
  columnId: Column['id'],
  columns: any,
): boolean {
  const index = columns.findIndex(({ id }: any) => id === columnId);
...
function getStickyProps(header: any, instance: any) {
  ...
  if (sticky) {
    ...
    const margin =
      sticky === 'left' ? header.totalLeft : getMarginRight(header.id, headers);
